...
class AsteriskConfigService:
    ...
    def generate_pjsip_config(self, phones: List[Dict[str, Any]]) -> str:
        ...
        IMPORTANT: Only processes phones with sip_* credentials (for WebSocket/outbound).
        Phones with only provider_sip_* credentials (for Zadarma/inbound) are handled
        separately in generate_zadarma_credentials().
        """
        config_lines = [
            ";==============================================================================",
            ...
            "; It defines endpoints for each configured phone number (WebSocket/outbound)",
            "; For Zadarma/inbound config, see pjsip_zadarma_credentials.conf",
            ";==============================================================================",
            ""
        ]
        
        # Filter to only include phones with valid sip_* credentials (for WebSocket/outbound)
        # Phones with only provider_sip_* are for Zadarma inbound and don't need endpoints here
        phones_with_sip = [
            p for p in phones 
            if p.get('sip_username') and p.get('sip_password') and p.get('sip_domain')
        ]
        
        logger.info(f"generate_pjsip_config: {len(phones)} total phones, {len(phones_with_sip)} with WebSocket SIP credentials")
        
        if not phones_with_sip:
            config_lines.extend([
                "; No phone numbers with WebSocket SIP configuration found",
                "; These endpoints are for WebSocket/WebRTC connections (outbound calls)",
                "; For Zadarma/inbound, configure provider_sip_* fields instead",
                ""
            ])
            return '\n'.join(config_lines)
        
        # Generate endpoint, auth, and AOR for each phone number with WebSocket SIP
        for phone in phones_with_sip:
        ...
        config_lines.extend([
            ...
            f"; Total endpoints configured: {len(phones_with_sip)}",
            f"; (Note: {len(phones) - len(phones_with_sip)} phones have provider SIP only - see zadarma credentials)",
            ";==============================================================================",
            ""
        ])
        ...
        if phones:
            ...
            for phone in phones:
                ...
                if provider_user and provider_pass:
                    ...
                    break
        logger.info(f"Generating Zadarma credentials==================: {sip_login} {sip_password} {sip_domain} {source}")
...
"""
    ...
    def write_config_files(self, pjsip_content: str, extensions_content: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Writing config files to {self.config_dir}")
            logger.info(f"PJSIP file: {self.pjsip_file} (exists: {self.pjsip_file.exists()})")
            logger.info(f"Extensions file: {self.extensions_file} (exists: {self.extensions_file.exists()})")
            logger.info(f"PJSIP content length: {len(pjsip_content)} bytes")
            logger.info(f"Extensions content length: {len(extensions_content)} bytes")
            
            # Create backup of existing files (after writing new content for safety)
            timestamp = datetime.utcnow().strftime('%Y%m%d_%H%M%S')
            
            # Write new config files first (safer - don't lose old file if write fails)
            # Write to temp file first, then rename
            pjsip_temp = self.pjsip_file.with_suffix('.conf.new')
            extensions_temp = self.extensions_file.with_suffix('.conf.new')
            
            pjsip_temp.write_text(pjsip_content)
            logger.info(f"Wrote PJSIP config to temp file {pjsip_temp}")
            
            extensions_temp.write_text(extensions_content)
            logger.info(f"Wrote extensions config to temp file {extensions_temp}")
            ...
            pjsip_temp.rename(self.pjsip_file)
            extensions_temp.rename(self.extensions_file)
            
            logger.info(f"Successfully wrote PJSIP config to {self.pjsip_file}")
            logger.info(f"Successfully wrote extensions config to {self.extensions_file}")
            
            return True
            
        except PermissionError as e:
            logger.error(f"Permission denied writing Asterisk config: {e}")
            logger.error(f"Config dir: {self.config_dir}, writable: {os.access(self.config_dir, os.W_OK)}")
            logger.error(f"PJSIP file writable: {os.access(self.pjsip_file, os.W_OK) if self.pjsip_file.exists() else 'N/A (does not exist)'}")
            return False
        except Exception as e:
            logger.error(f"Error writing Asterisk config files: {e}", exc_info=True)
            return False
    ...
    def regenerate_config(self, db: Optional[Session] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Log details about each phone for debugging
            phones_with_websocket_sip = 0
            phones_with_provider_sip = 0
            for p in phones:
                has_ws = bool(p.get('sip_username') and p.get('sip_password') and p.get('sip_domain'))
                has_provider = bool(p.get('provider_sip_username') and p.get('provider_sip_password'))
                if has_ws:
                    phones_with_websocket_sip += 1
                if has_provider:
                    phones_with_provider_sip += 1
                logger.info(f"Phone {p.get('phone_number')}: websocket_sip={has_ws}, provider_sip={has_provider}, agent_id={p.get('agent_id')}")
            
            result['phones_with_websocket_sip'] = phones_with_websocket_sip
            result['phones_with_provider_sip'] = phones_with_provider_sip
            
            if not phones:
                logger.warning("No phone numbers with any SIP config found")
                result['errors'].append("No phone numbers with SIP configuration found")
                # Still generate basic config even with no phones
            else:
                logger.info(f"Found {len(phones)} phones: {phones_with_websocket_sip} with WebSocket SIP (for pjsip_weevoice.conf), {phones_with_provider_sip} with provider SIP (for zadarma credentials)")
