...
from app.api import auth, agents, calls, websocket, billing, usage, security, profile, documents, phone_numbers, callbacks, embed, zadarma_webhook, admin, integrations, libraries, collaborators, phone_audio, phone_debug, performance, eagi, virtual_numbers, asterisk, outbound_scripts
...
async def lifespan(app: FastAPI):
    ...
    # PRE-WARM greetings using GEMINI BEFORE accepting any calls
    # This ensures instant greeting playback with SAME VOICE as conversation
    print("[GREETING] 🔥 Pre-warming greeting cache using Gemini...")
    try:
        from app.services.greeting_tts_service import prewarm_all_agent_greetings, GEMINI_AVAILABLE
        if GEMINI_AVAILABLE:
            await prewarm_all_agent_greetings()
            print("[GREETING] ✅ Cache pre-warmed - same voice for greeting & conversation!")
        else:
            print("[GREETING] ⚠ Gemini not available for greeting pre-generation")
    except Exception as e:
        print(f"[GREETING] ⚠ Pre-warm failed: {e}")
        logger.warning(f"Greeting pre-warm failed: {e}")
    
    # AudioSocket server - provides smooth audio streaming for phone calls
    # AudioSocket uses direct TCP streaming (no file I/O) = better than EAGI
...
app.include_router(virtual_numbers.router, prefix=f"{settings.API_V1_STR}/virtual-numbers", tags=["Virtual Numbers"])
app.include_router(asterisk.router, prefix=f"{settings.API_V1_STR}/asterisk", tags=["Asterisk Configuration"])
app.include_router(outbound_scripts.router, prefix=f"{settings.API_V1_STR}", tags=["Outbound Scripts"])
