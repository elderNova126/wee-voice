...
if command -v asterisk &> /dev/null; then
    ...
    # Create empty include files (will be generated by backend when phone numbers are configured)
    touch "$ASTERISK_CONF/pjsip_weevoice.conf"
    touch "$ASTERISK_CONF/extensions_weevoice.conf"
    touch "$ASTERISK_CONF/pjsip_zadarma_credentials.conf"
    ...
    log_info "✓ Empty include files created (will be populated by backend)"
